#ifndef ZERO_OVERHEAD_BUFFER_MANAGER_HPP
#define ZERO_OVERHEAD_BUFFER_MANAGER_HPP
...
public:
    ...
    // Non-copyable
    ZeroOverheadManager(const ZeroOverheadManager&) = delete;
    ZeroOverheadManager& operator=(const ZeroOverheadManager&) = delete;
...
/* --------------------------------------------------------------------------
 * ZeroOverheadContext - lightweight RAII around manager/pool selection
 * ----------------------------------------------------------------------- */
class ZeroOverheadContext {
public:
    ZeroOverheadContext(int max_atoms, int max_neighbors, int species_count = 1);
    ~ZeroOverheadContext();
...
private:
    ZeroOverheadManager* manager = nullptr;
    bool initialized = false;
};

/* --------------------------------------------------------------------------
 * OverheadProfiler - simple profiler used by the pair-style for breakdowns
 * ----------------------------------------------------------------------- */
...
public:
    ...
    class ScopedTimer {
    public:
        ScopedTimer(OverheadProfiler* prof, const std::string& name);
        ~ScopedTimer();
    private:
    ...
    OverheadProfiler(const OverheadProfiler&) = delete;
    OverheadProfiler& operator=(const OverheadProfiler&) = delete;
...
#define PROFILE_SCOPE(profiler, name) \
    OverheadProfiler::ScopedTimer _timer(profiler, name)

/* -------------------------------------------------------------------------- */
} // namespace ZeroOverhead

#endif // ZERO_OVERHEAD_BUFFER_MANAGER_HPP
