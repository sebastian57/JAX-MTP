...
public:
    ...
    // Input validation to prevent crashes and handle edge cases
    bool validate_input_sizes(int actual_atoms, int max_neighbor_count);
...
public:
    ...
    bool call_jax_ultra_optimized(
        const std::string& function_path,
        int natoms_energy,        // Number of local atoms for energy calculation
        int natoms_force,         // Number of atoms for force calculation (usually same as natoms_energy)
        const double* const* atom_positions,
        const int* atom_types,
        const int* const* neighbor_lists,
        const int* neighbor_counts,
