...
void PersistentMemoryPool::update_atom_data_zero_copy(
    ...
    try {
        ...
        for (int i = 0; i < atoms_to_copy; i++) {
            // CRITICAL: Atom types (1-based to 0-based conversion)
            int atom_type_raw = atom_types[i];
            int atom_type_converted = atom_type_raw - 1;
            ...
            if (atom_type_converted < 0) {
                std::cout << "⚠️ WARNING: Negative converted atom type " << atom_type_converted << " for atom " << i << std::endl;
            }
            #endif
            
            itypes_data[i] = atom_type_converted;

            // Number of neighbors to copy for this atom (bounded)
            int neighbors_to_copy = std::min(neighbor_counts[i], config.max_neighbors);
            
            // STABILITY CHECK: Neighbor list truncation warning
            #ifdef ZO_DEBUG
            if (neighbor_counts[i] > config.max_neighbors) {
                std::cout << "⚠️ STABILITY WARNING: Truncating neighbor list for atom " << i 
                          << " (" << neighbor_counts[i] << " -> " << config.max_neighbors << ")" << std::endl;
            }
            #endif
            ...
            for (int j = 0; j < neighbors_to_copy; j++) {
                ...
                if (j < neighbor_counts[i]) {
                    float rx = (float)atom_positions[i][j * 3 + 0];
                    float ry = (float)atom_positions[i][j * 3 + 1];
                    float rz = (float)atom_positions[i][j * 3 + 2];
                    ...
                    if (distance > 15.0f) { // Assuming max reasonable distance
                ...
                if (j < neighbor_counts[i]) {
                    ...
                    int converted_neighbor_type = raw_neighbor_type - 1; // Convert to 0-based types
                    ...
                    if (converted_neighbor_type < 0) {
                        std::cout << "⚠️ WARNING: Negative converted neighbor type " << converted_neighbor_type 
                                  << " for atom " << i << " neighbor " << j << std::endl;
                    }
                    #endif
...
bool ZeroOverheadOriginalManager::call_jax_ultra_optimized_rectangular(
    ...
    try {
        ...
        bool success = call_jax_ultra_optimized(
            function_path,
            actual_atoms,
            max_neighbors,  // Use max_neighbors as the effective neighbor count
