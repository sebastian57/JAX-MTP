/* ----------------------------------------------------------------------
   ...
   - FIXED itypes padding to correctly handle padded atoms.
   - ADDED robust safety checks for handling JAX return values.
   - RESTORED detailed debugging output for array validation.
------------------------------------------------------------------------- */
...
// --- Boilerplate: Singleton, NumPy Init, Destructor ---
    ...
    const int* const* neighbor_types)
{
    ...
    // --- Pre-fill neighbor arrays with padding values ---
    ...
    // #################### FIX: CORRECT ITYPES PADDING ####################
    // The itypes array must also be padded for atoms beyond natoms_actual.
    // First, fill the entire array with the padding value.
    for (int i = 0; i < config_max_atoms; ++i) {
        itypes_data[i] = type_sentinel;
    }
    // Then, copy over only the real atom types.
    if (natoms_actual > 0) {
      std::memcpy(itypes_data, atom_types, natoms_actual * sizeof(int32_t));
    }
    // #################### END ITYPES PADDING FIX ####################

    // --- Copy Neighbor Data from LAMMPS ---
...
void PersistentMemoryPool::cleanup() {
    // If Python is not initialized, avoid calling any Python C-API functions
    // (Py_XDECREF would crash if the interpreter is already finalized).
    if (!Py_IsInitialized()) {
        // Just null the pointers and mark pool uninitialized.
    ...
    // Python is alive — safely acquire GIL when touching Python objects.
...
ZeroOverheadOriginalManager* ZeroOverheadOriginalManager::get_instance() {
    ...
    if (!instance) {
        instance = std::unique_ptr<ZeroOverheadOriginalManager>(new ZeroOverheadOriginalManager());
    }
    return instance.get();
}

ZeroOverheadOriginalManager::~ZeroOverheadOriginalManager() { cleanup(); }

bool ZeroOverheadOriginalManager::initialize_for_system(int max_atoms, int max_neighbors) {
...
bool ZeroOverheadOriginalManager::execute_potential(
    ...
    double& energy, double** forces, double* stress)
{
    ...
    *(int32_t*)PyArray_DATA((PyArrayObject*)active_pool->get_nneigh_total_obj()) = nneigh_total;
    ...
    PyObject* result = PyObject_CallFunctionObjArgs(
        ...
        active_pool->get_natoms_actual_obj(), // Note: nneigh_total_obj was passed here before, but the arg name implies natoms
        nullptr
    );
    
    // ###################### ROBUST SAFETY CHECKS (FIXED) ######################
    bool success = true;
    if (!result) {
        std::cerr << "❌ FATAL: JAX function call returned NULL. Python exception occurred." << std::endl;
        PyErr_Print();
        success = false;
        ...
        // --- END JAX to NumPy Conversion ---

        if (!forces_np_obj || !stress_np_obj) {
            PyErr_Print();
            std::cerr << "❌ FATAL: Failed to convert JAX arrays to NumPy arrays." << std::endl;
            success = false;
        // --- MODIFIED: Use converted NumPy objects for checks and data access ---
        } else if (!PyArray_Check(forces_np_obj) || !PyArray_Check(stress_np_obj)) {
            std::cerr << "❌ FATAL: Returned forces or stress object is not a NumPy array even after conversion." << std::endl;
            success = false;
        } else {
            ...
            // Ensure data types match what LAMMPS expects (double)
    ...
    Py_XDECREF(result);
    // #################### END ROBUST SAFETY CHECKS (FIXED) ####################
    
    PyGILState_Release(gstate);
    return success;
}

bool ZeroOverheadOriginalManager::load_and_cache_jax_function(const std::string& function_path) {
...
void ZeroOverheadOriginalManager::cleanup() {
    // If Python is not initialized we must not call PyGILState_Ensure / Py_XDECREF.
    if (!Py_IsInitialized()) {
        // Clear cached pointers (we cannot DECREF safely now).
...
ZeroOverheadOriginalContext::ZeroOverheadOriginalContext(int max_atoms, int max_neighbors) {
